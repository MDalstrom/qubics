...
from application.display import create_export_system, create_interactive_system, duration_system, handle_events
from application.rendering.bounds import bounds_render_system
from application.rendering.circle import renderable_system
...
def _get_base_scenario(
    ...
    return Scenario(
        ...
        SystemsGroup(
            ...
            [bounds_render_system, renderable_system],
            [create_export_system(writer_fn()) if config.get('output') is not None else create_interactive_system()]
        )
    )
