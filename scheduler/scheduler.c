...
struct Scheduler {
  size_t count;
  InternalDescriptor *descriptors;
};
...
static bool tags_overlap(const void **a, size_t ac, const void **b, size_t bc) {
...
static bool systems_conflict(InternalDescriptor *a, InternalDescriptor *b) {
  return tags_overlap(a->writes, a->write_count, b->reads,  b->read_count)  ||
         tags_overlap(b->writes, b->write_count, a->reads,  a->read_count)  ||
         tags_overlap(a->writes, a->write_count, b->writes, b->write_count);
}

static size_t systems_sort(InternalDescriptor *systems, size_t count, size_t *stage_ends) {
  ...
  for (size_t i = 1; i < count; i++) {
    InternalDescriptor sys = systems[i];
...
void scheduler_destroy(Scheduler *scheduler) {
  for (size_t i = 0; i < scheduler->count; i++) {
    free((void *)scheduler->descriptors[i].reads);
    free((void *)scheduler->descriptors[i].writes);
  }
...
void scheduler_add(Scheduler *scheduler, void (*fn)(void *world, void *ctx),
  ...
  scheduler->descriptors = realloc(
      ...
      ++scheduler->count * sizeof(InternalDescriptor));

  InternalDescriptor *d = &scheduler->descriptors[scheduler->count - 1];
  d->fn = fn;

  d->read_count = reads.count;
  d->reads = reads.count ? malloc(reads.count * sizeof(void *)) : NULL;
  if (reads.count) memcpy((void *)d->reads, reads.tags, reads.count * sizeof(void *));

  d->write_count = writes.count;
  d->writes = writes.count ? malloc(writes.count * sizeof(void *)) : NULL;
  if (writes.count) memcpy((void *)d->writes, writes.tags, writes.count * sizeof(void *));
}

Runner *runner_create(Scheduler *scheduler) {
  size_t count = scheduler->count;

  InternalDescriptor *sorted = malloc(count * sizeof(InternalDescriptor));
  memcpy(sorted, scheduler->descriptors, count * sizeof(InternalDescriptor));
...
void runner_tick(Runner *runner, void *world, void *ctx) {
  ...
  for (size_t s = 0; s < runner->stages_count; s++) {
    ...
    for (int t = 0; t < SCHEDULER_THREADS; t++) {
      args[t] = (TickArgs){ runner, world, ctx, end, &cursor };
